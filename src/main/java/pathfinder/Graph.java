...
public class Graph  {
	...
	protected HashMap<Long, LinkedList<FloatingEdge>> nodesToBe;
	...
	public Graph(int nbrNodes){
		nodes = new HashMap<Long, GraphNode>(nbrNodes);
	 ...
	 */
	public boolean addEdge(long fromID, long toID, double cost){
	 ...
	 * @return true if the edge was added else false
	 */
	public boolean addEdge(long fromID, long toID, double costOutward, double costInward){
		boolean added = false;
	 ...
	 * @param floatEdge
	 */
	protected void rememberFloatingEdge(long id, FloatingEdge floatEdge){
		if(nodesToBe == null)
			nodesToBe = new HashMap<Long, LinkedList<FloatingEdge>>();
...
//	}
	 ...
	 */
	public GraphEdge getEdge(long fromID, long toID){
		GraphNode fromNode = nodes.get(fromID);
		GraphNode toNode = nodes.get(toID);
		if(fromNode == null || toNode == null)
			return null;
	 ...
	 * @return the edge or null if it doesn't exist
	 */
	public double getEdgeCost(long fromID, long toID){
		GraphNode fromNode = nodes.get(fromID);
		GraphNode toNode = nodes.get(toID);
		if(fromNode == null || toNode == null)
			return -1;
	 ...
	 */
	public boolean removeEdge(long fromID, long toID){
	 ...
	 */
	public LinkedList<GraphEdge> getEdgeList(long nodeID){
	...
	/**
	 * Gets a list of GraphEdges to this node. <br>
	 * Not used by graph searches, purely for traffic sim project.
	 * @author ae9is
	 * @param nodeID id of the node where edges end at
	 */
	public LinkedList<GraphEdge> getEdgeListTo(long nodeID) {
		GraphEdge[] edges = getAllEdgeArray();
		LinkedList<GraphEdge> edgesTo = new LinkedList<>();
		for(int i = 0; i < edges.length; i++){
			if (edges[i].to().id() == nodeID) {
				edgesTo.add(edges[i]);
			}
	 ...
	 */
	public GraphEdge[] getEdgeArray(long from){
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <T extends GraphEdge> T[] getEdgeArray(long from, T[] array){
		if(array == null)
			array = (T[]) new Object[0];
	...
	private class FloatingEdge {
		 ...
		 * @param cost
		 */
		public FloatingEdge(long fromID, long toID, GraphNode from, GraphNode to,
				double cost) {
			super();
			this.fromID = fromID;
			this.toID = toID;
